...
Binaries are placed in `docs/mingw-example/build/<arch>/` by the `dist-builder` Docker Compose service.
...
execute_process(COMMAND which x86_64-w64-mingw32-windres
  OUTPUT_VARIABLE TOOLCHAIN_RC_COMPILER)
execute_process(COMMAND which x86_64-w64-mingw32-dlltool
  OUTPUT_VARIABLE TOOLCHAIN_DLLTOOL)
